...
def add_sentence_context(df, sentence_col="Sentence", window=1):
    sentences = df[sentence_col].tolist()
    context_list = []

    for i in range(len(sentences)):
        left = sentences[i - window] if i - window >= 0 else ""
        right = sentences[i + window] if i + window < len(sentences) else ""
        center = sentences[i]
        combined = f"{left} {center} {right}".strip()
        context_list.append(combined)

    df["SentenceWithContext"] = context_list
    return df

df = add_sentence_context(df, sentence_col="Sentence", window=1)
print(df[["Sentence", "SentenceWithContext"]].head())
...
#%% --- Example Usage ---
test_df["matched_entities"] = process_all_rows_grouped(test_df, "SentenceWithContext")
...
# %%
test_df.to_csv("Sentence(B&A)_outcome.csv",index=False)
